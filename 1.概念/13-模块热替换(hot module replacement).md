...
通过以下步骤，可以做到应用程序置换 (swap in and out) 模块：

- 引用程序代码要求 HMR runtime 检测更新;
- HMR runtime (异步) 下载更新，然后通知应用程序代码;
- 应用程序代码要求 HMR runtime 应用更新;
- HMR runtime (同步) 应用更新。

你可以设置HMR ， 以使用进程自动触发更新，或者你可以选择要求在用户交互时进行更新。

### 在编译器中

除了普通资源，编译器 (compiler) 需要发出 "update", 以允许更新之前的版本到新版本。"update" 由两个部分组成：
...
HMR 是可选功能，只影响包含HMR 代码的模块。

例如：通过 `style-loader` 为 style 样式追加补丁。为了运行追加补丁，`style-loader` 实现了 HMR 接口；当它通过 HMR 接收到更新，它会使用新的样式替换旧的样式。
